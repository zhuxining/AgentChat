...
- AgentChat 持久化用户消息和最终 Agent 结果；Pi 持久化自己的会话执行细节。
- Pi 工具在 macOS 用户权限下执行。Agent 配置的工作目录是默认上下文，不是强制安全隔离边界。
- 首发只支持一个 Agent 对应一个 Conversation；同一 Conversation 中的消息按序处理。
...
- [当前目录结构](99-directory-structure.md)
- [依赖清单](reference/dependencies.md)
- [当前配置](reference/config.md)

## 核心设计原则

1. **Host 是产品事实来源：** Host 保存 AgentChat 的会话消息与最终结果；客户端离线缓存只用于只读展示，不覆盖 Host 数据。
...
| AgentChat 与 Pi 分别持久化什么？     | Host 用 SQLite 保存产品消息和最终结果；Pi 保存执行会话与工具细节。       | Host 复制完整 Pi 事件流、仅依赖 Pi 历史展示、持久化完整 Run/Task 状态。 | 保留多端可读的产品历史，同时避免重复保存执行轨迹和恢复状态机。                           | 需要精确恢复任务状态、审计工具行为或跨设备回放时。   |
...
- **User：** 单个本机 AgentChat 安装的所有者。
- **Device：** 经二维码配对并可单独撤销的客户端设备。
- **Agent：** 用户选择的角色配置，包含 Pi 工作上下文和产品身份。
- **Conversation：** 与一个 Agent 关联的持续对话，并对应一个 Pi Session。
- **Message：** 用户输入或 Agent 最终响应；Host 持久化消息内容与结果。
- **Pi Session：** Pi 拥有并持久化的执行上下文；Host 为 Conversation 选择对应 Session，但不复制其工具调用明细。
...
erDiagram
    ...
    CONVERSATION ||--|| PI_SESSION : maps_to
```
...
桌面 Tauri 进程同时承载 UI、Rust Host 和局域网 API。Host 关闭窗口后留在菜单栏后台；Host 显式退出时关闭由其管理的 Pi RPC 子进程。每个活动 Conversation 使用自己的 Pi Session；Host 对活动 Pi 进程采用有限并发和近期进程保留策略。
...
- AgentChat 不提供应用层静态加密、备份或导出；本机消息与结果以普通应用数据保存。
